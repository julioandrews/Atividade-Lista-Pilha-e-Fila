...
bool inserirElemListaOrd(LISTA* l, REGISTRO reg) {
  ...
  if (ant == NULL) {
    i->ant = NULL;
    i->prox = l->inicio;
    if (l->inicio != NULL) l->inicio->ant = i;  // Só ajusta o ant do próximo se a lista NÃO estiver vazia
    l->inicio = i;
  } else {
    i->ant = ant;
    i->prox = ant->prox;
    if (ant->prox != NULL) ant->prox->ant = i;  // Só ajusta o ant do próximo se NÃO for o final da lista
    ant->prox = i;
  }
  return true;
}
